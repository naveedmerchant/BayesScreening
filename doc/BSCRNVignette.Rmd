...
If the user is less interested in how bandwidths are chosen from the posterior, we recommend just calling PredCVBFMHbw to get a list of bandwidths and relying on the default values. 

Whatever the case, we now illustrate the methods to draw from the posterior bandwidth distribution.
...
The training_ids do not need to be provided, instead a seed can be provided to ensure reproducibility instead. The minimum input required to run the test are the data sets and some size they'd like the training set to be. 
