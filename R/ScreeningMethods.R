...
ParScreenVars <- function(datasetX, datasetY, method = "SIS", ncores = 1, cutoff = NULL, train1ids = NULL, trainsize1 = NULL, trainsize2 = NULL, train2ids = NULL, seed = NULL, Ginv = NULL, c = NULL, leveltot = NULL, PTscale = TRUE)
{
  ...
  if(method == "SIS")
  {
    ...
    cl <- makeCluster(ncores) 
    registerDoParallel(cl)
    logBFlist <- foreach(j=1:p, .combine=c, .export = c("CVBFtestrsplit", "HallKernel", "laplace.kernH2c", "KHall", "logpriorused", "logintegrand.Hall", "loglike.KHall"), .packages = "rootSolve") %dopar% {
...
SeqScreenVars <- function(datasetX, datasetY, method = "SIS", cutoff = NULL, train1ids = NULL, trainsize1 = NULL, trainsize2 = NULL, train2ids = NULL, seed = NULL, Ginv = NULL, c = NULL, leveltot = NULL, PTscale = TRUE)
{
  ...
  if(method == "SIS")
  {
    ...
    pvaluelist <- foreach(j=1:p, .combine=c) %do% {
      tempMatrix = t.test(datasetX[Class0ind,j], datasetX[-Class0ind,j])$p.value 
    ...
    logBFlist <- foreach(j=1:p, .combine=c, .export = c("CVBFtestrsplit", "HallKernel", "laplace.kernH2c", "KHall", "logpriorused", "logintegrand.Hall", "loglike.KHall", "integrand.Hall"), .packages = "rootSolve") %do% {
      tempMatrix = CVBFtestrsplit(datasetX[Class0ind, j], datasetX[-Class0ind, j], trainsize1, trainsize2, seed = seed, train1_ids = train1ids, train2_ids = train2ids)$logBF 
    ...
    #browser()
    logBFlist <- foreach(j=1:p, .combine=c, .export = c("PolyaTreetest")) %do% {
      tempMatrix = PolyaTreetest(datasetX[Class0ind, j], datasetX[-Class0ind, j], Ginv = Ginv, c = c, leveltot = leveltot)$logBF 
